...
fn main() {
    let node_map = get_nodes();
    let mut seen_before = HashSet::new();

    let zero_node = node_map.get(&0).unwrap();
    get_connected_nodes(&mut seen_before, &node_map, zero_node);

    println!("group size of 0 is: {}", seen_before.len());
}
